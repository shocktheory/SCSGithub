...
export const assignmentDirectives: AssignmentDirective[] = [
  ...
  // operational-history) — NO decision record, NO implementation records. Canonical ST-ADR id pending.
  {
    ...
    status: 'Active — planning package submitted to the Phase 7 Authorization Package Review gate; awaiting Product Owner disposition',
    standingDirective: 'sdr-002', deliverable: 'dlv-011', reviewGate: 'rgate-011',
    authorityStatus: 'approved', demonstration: false,
    notes: 'Planning and authorization-preparation ONLY. Defines every architectural/functional/governance/verification/acceptance aspect of Phase 7 (server-side constitutional derivation & canonical state authority) so the Product Owner can decide whether Phase 7 begins. Authorizes NO implementation; implements NO server-side derivation; NO deployment/hosting/confidential data/launch. Per the directive\'s closed list, only planning records were created (no decision record, no implementation records). Derives from accepted Baseline v1.0, Completion Program Rev 2, Phase 6 Implementation, and Authentication & Authority Principles. No canonical identifiers originated. See SCS_PHASE_7_AUTHORIZATION_PACKAGE.md.',
  },
];
...
export const deliverables: Deliverable[] = [
  ...
  { id: 'dlv-011', deliverableId: 'ST-DLV-2026-011', title: 'SCS Phase 7 Authorization Package (planning)', assignmentDirective: 'adr-012', reviewGate: 'rgate-011', status: 'In review — Phase 7 Authorization Package Review (planning document; no implementation)', authorityStatus: 'reported', demonstration: false, notes: 'Planning-only authorization package for Phase 7 — Server-Side Constitutional Derivation & Canonical State Authority. Contents: executive overview; capability breakdown (Domain→Capability→Requirements→Evidence) across 10 areas; constitutional derivation architecture; complete governed command model (propose…retire, defined not implemented); client/server responsibility matrix; constitutional state transition model; derivation version strategy; parity strategy; threat/risk & security review; verification strategy (defined, NOT executed); traceability; updated Product Owner decision queue (11 items); Phase 7 readiness findings (no self-recommendation). Authorizes NO implementation. #SCS does not self-authorize. See SCS_PHASE_7_AUTHORIZATION_PACKAGE.md.' },
];
...
export const reviewGates: Gate[] = [
  ...
  { id: 'rgate-011', name: 'Phase 7 Authorization Package Review', requiresOwnerApproval: true, status: 'Open — pending Product Owner review (planning package; Phase 7 implementation NOT authorized)', authorityStatus: 'proposed', demonstration: false },
];
